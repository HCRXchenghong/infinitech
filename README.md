...
- Go readiness 和 BFF 健康聚合已能看到 push worker 状态、最近成功时间、连续失败次数与队列积压。
...
- 2026-03-29：用户端和 App 端消息首页已重写成干净 UTF-8，继续保持“服务端会话优先、本地缓存仅兜底”的上线口径。
- 2026-03-29：后台首页运维辅助文案源已清理，统计卡片、Redis 模式、在线样本、fallback 年龄等提示恢复成正常中文。
