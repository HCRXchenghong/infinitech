...
- 骑手端全局消息管理器现在会先规范化 `chatId` 再判断当前会话，避免同一会话因数字/字符串类型不一致而误触发弹窗或跳错聊天页
- 商家聊天页现在也会在缺少 `time` 文本时回退到服务端 `timestamp / createdAt` 计算显示时间，避免历史消息被错误显示成“当前时刻”
- 用户端与 App 端聊天页现在也会在缺少 `time` 文本时回退到服务端 `timestamp / createdAt` 计算显示时间，并按前一条消息的真实时间决定是否展示时间分隔
- `socket-server /api/stats` 现在除了 Redis 口径在线人数，还会返回一份 Redis 优先的在线样本列表，方便多实例场景下继续排查实时服务在线态
- 后台首页现在会直接展示 Redis 共享在线连接样本，并且 `server_stats` 实时推送也会带上在线样本字段，方便持续观察多实例实时网关的在线态
- 用户端、App 端和骑手端客服页现在也会正确解析服务端 `createdAt` 字符串，不再把历史客服消息错误回退成当前本地时间
- 骑手端全局实时分发与弹窗入口也已统一按服务端 `createdAt / timestamp` 解析时间，避免骑手收到商家/用户消息时再次被本地当前时间覆盖
- 骑手端本地 SQLite 消息缓存现在也会按服务端时间解析并生成稳定兜底消息 ID，减少缓存层把历史顺序和去重再次带偏
- `socket-server` 本地 `chat.db` 兜底层已新增独立 `event_timestamp`，客服会话列表和历史消息在回退本地数据库时也会优先按原始消息时间排序，不再只依赖 SQLite 插入时间
