...
- 用户端与 App 端聊天页里已删除废弃的本地 `user_message_sessions_*` 会话写入逻辑，继续压缩本地伪会话来源
- 用户端、App 端和商家端聊天页现在改为“服务端历史成功即覆盖本地缓存，失败才回落本地缓存”，并且会在后续 socket 历史回补到达时自动替换本地兜底数据
- 用户端与 App 端消息首页现在会优先解析服务端 `updatedAt / lastMessageAt / latestAt / timestamp` 等时间字段，不再因为缺少时间戳就本地补 `Date.now()`；点进会话时也只临时清本地未读，不再本地篡改会话更新时间
- 后台客服工作台现在也开始保留服务端 `updatedAt`，本地收到新消息后会按服务端时间重新排序；选中会话时只临时清本地未读，不再直接改原会话对象
- 骑手客服页和后台客服工作台现在也改为“服务端历史成功即覆盖本地缓存，失败才回落本地缓存”，骑手端还会在服务端历史回写时按会话清理旧 SQLite 缓存
...
- 骑手端、后台工作台剩余本地伪未读和零散本地辅助状态还需继续清理
