<template>
  <view class="page category-page">
    <PageHeader title="超市便利" :showSearch="true" @search="goSearch" />
...
export default {
  ...
  methods: {
    matchCategory(shop) {
      ...
      if (category.includes('超市') || category.includes('便利') || category.includes('超市便利')) {
        return true
      }
      return tags.some(tag => tag.includes('超市') || tag.includes('便利') || '超市便利'.includes(tag))
    },
    async loadShops() {
