import { fetchAfterSalesList, fetchGroupbuyVouchers, fetchOrders, fetchVoucherQRCode } from '@/shared-ui/api.js'
  ...
  methods: {
    ...
    handleOnlineContact() {
      ...
      const name = this.contactType === 'rider'
        ? (order.deliveryInfo?.rider?.split(' ')[0] || '骑手')
        : order.shopName
    ...
    handleConfirmPhone() {
      ...
      if (this.contactType === 'rider') {
        phoneNumber = order.deliveryInfo?.rider?.match(/\d{11}/)?.[0] || order.riderPhone || '10086'
      } else {
