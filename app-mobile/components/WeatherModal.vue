      ...
      <view class="modal-body">
        <view class="weather-main">
          ...
          <view class="weather-info">
            ...
            <text class="location-text">{{ weather.province || '' }} {{ weather.city || '' }}</text>
          </view>
        </view>
        <view class="weather-extra">
          <view class="extra-item">
            <text class="extra-label">体感</text>
            <text class="extra-value">{{ weather.feelsLike || weather.temp }}°</text>
          </view>
          <view class="extra-item">
          ...
          <view class="extra-item" v-if="weather.windDirection || weather.windPower">
            <text class="extra-label">风力</text>
            <text class="extra-value">{{ weather.windDirection || '' }} {{ weather.windPower || '' }}</text>
          </view>
          ...
          <view class="extra-item" v-if="weather.refreshIntervalMinutes">
            ...
            <text class="extra-value">{{ weather.refreshIntervalMinutes }} 分钟</text>
          </view>
          <view class="extra-item">
            <text class="extra-label">提示</text>
            <text class="extra-value">{{ weather.tips || '出门记得带伞，多喝热水～' }}</text>
          </view>
        </view>

        <view class="section-block" v-if="forecastList.length > 0">
          <text class="section-title">多天预报（{{ forecastList.length }}）</text>
          <view v-for="(item, idx) in forecastList" :key="'fc-' + idx" class="kv-row">
            <text class="kv-key">{{ item.date || item.fxDate || ('第' + (idx + 1) + '天') }}</text>
            <text class="kv-value">
        ...
        <view class="section-block" v-if="hourlyList.length > 0">
          <text class="section-title">逐小时预报（{{ hourlyList.length }}）</text>
          <view v-for="(item, idx) in hourlyPreviewList" :key="'hour-' + idx" class="kv-row">
            <text class="kv-key">{{ item.fxTime || item.time || item.datetime || ('+' + idx + 'h') }}</text>
            <text class="kv-value">{{ item.weather || item.text || '--' }} {{ resolveHourlyTemp(item) }}°</text>
          </view>
        </view>
        ...
        <view class="section-block" v-if="lifeIndexList.length > 0">
          <text class="section-title">生活指数（{{ lifeIndexList.length }}）</text>
          <view v-for="(item, idx) in lifeIndexList" :key="'idx-' + idx" class="kv-row">
...
export default {
  ...
  computed: {
    rawWeather() {
      return this.weather && this.weather.raw ? this.weather.raw : {}
    },
    forecastList() {
      const value = this.rawWeather.forecast
      return Array.isArray(value) ? value : []
    },
    hourlyList() {
      const value = this.rawWeather.hourly_forecast
      return Array.isArray(value) ? value : []
    },
    hourlyPreviewList() {
      return this.hourlyList.slice(0, 12)
    },
    minutelyList() {
      const value = this.rawWeather.minutely_precip && this.rawWeather.minutely_precip.data
      return Array.isArray(value) ? value : []
    },
    minutelySummary() {
      return (this.rawWeather.minutely_precip && this.rawWeather.minutely_precip.summary) || ''
    },
    lifeIndexList() {
      const raw = this.rawWeather.life_indices
    ...
    weatherJson() {
      try {
        return JSON.stringify(this.rawWeather || this.weather || {}, null, 2)
      } catch (e) {
        return '{}'
      }
    },
  ...
  methods: {
    resolveForecastMin(item = {}) {
      const value = item.temp_min !== undefined ? item.temp_min : (item.tempMin !== undefined ? item.tempMin : item.nighttemp)
      return value !== undefined && value !== null && value !== '' ? value : '--'
    },
    resolveForecastMax(item = {}) {
      const value = item.temp_max !== undefined ? item.temp_max : (item.tempMax !== undefined ? item.tempMax : item.daytemp)
      return value !== undefined && value !== null && value !== '' ? value : '--'
    },
