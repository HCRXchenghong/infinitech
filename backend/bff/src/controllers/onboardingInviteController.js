...
async function adminListInvites(req, res, next) {
  await proxyGet(req, res, next, "/admin/onboarding/invites", {
    params: req.query,
  });
}
...
async function adminRevokeInvite(req, res, next) {
  await proxyPost(
    ...
    res,
    next,
    `/admin/onboarding/invites/${req.params.id}/revoke`,
  );
}

async function adminListSubmissions(req, res, next) {
  await proxyGet(req, res, next, "/admin/onboarding/invites/submissions", {
    params: req.query,
  });
}
...
async function publicSubmitInvite(req, res, next) {
  await proxyPost(
    ...
    res,
    next,
    `/onboarding/invites/${req.params.token}/submit`,
  );
}

async function publicUploadAsset(req, res, next) {
  await proxyMultipartUpload(req, res, next, {
    path: `/onboarding/invites/${req.params.token}/upload`,
  });
}
