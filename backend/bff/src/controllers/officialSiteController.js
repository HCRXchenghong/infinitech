...
const { proxyGet, proxyPost, proxyPut, requestGoRaw } = require('../utils/goProxy');
...
async function proxyOfficialSite(req, res, next, options = {}) {
  try {
    ...
    return res.status(response.status).json(normalizeOfficialSitePayload(response.data, req));
  } catch (error) {
    if (error.response) {
      return res.status(error.response.status).json(normalizeOfficialSitePayload(error.response.data, req));
    }
    return next(error);
  }
}
...
async function getSupportSocketToken(req, res, next) {
  ...
  if (!apiSecret) {
    return res.status(503).json({ success: false, error: 'socket token issuance is not configured' });
  }
  ...
  try {
    ...
    if (sessionResponse.status < 200 || sessionResponse.status >= 300) {
      return res.status(sessionResponse.status).json(sessionResponse.data || {
        success: false,
    ...
    if (!session || !sessionToken) {
      return res.status(404).json({ success: false, error: 'support session not found' });
    }
...
async function listAdminExposures(req, res, next) {
  await proxyGet(req, res, next, '/admin/official-site/exposures', { params: req.query });
}

async function updateExposure(req, res, next) {
  await proxyPut(req, res, next, `/admin/official-site/exposures/${req.params.id}`);
}

async function listAdminCooperations(req, res, next) {
  await proxyGet(req, res, next, '/admin/official-site/cooperations', { params: req.query });
}

async function updateCooperation(req, res, next) {
  await proxyPut(req, res, next, `/admin/official-site/cooperations/${req.params.id}`);
}

async function listAdminSupportSessions(req, res, next) {
  await proxyGet(req, res, next, '/admin/official-site/support/sessions', { params: req.query });
}

async function getAdminSupportMessages(req, res, next) {
  await proxyGet(req, res, next, `/admin/official-site/support/sessions/${req.params.id}/messages`);
}

async function appendAdminSupportMessage(req, res, next) {
  await proxyPost(req, res, next, `/admin/official-site/support/sessions/${req.params.id}/messages`);
}

async function updateSupportSession(req, res, next) {
  await proxyPut(req, res, next, `/admin/official-site/support/sessions/${req.params.id}`);
}
