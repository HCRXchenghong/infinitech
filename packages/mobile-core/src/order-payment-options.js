export function normalizePayChannel(raw) {
  ...
  if (value === "if-pay" || value === "if_pay" || value === "balance") {
    return "ifpay";
  }
  if (value === "wxpay" || value === "wechatpay") {
    return "wechat";
  }
  if (value === "ali") {
    return "alipay";
...
export function fallbackOrderPayMethods({ platform = "app" } = {}) {
  const fallback = [
    { value: "ifpay", label: "IF-Pay 余额支付", tip: "优先使用钱包余额" },
    {
      ...
      label: "微信支付",
      tip:
        platform === "mini_program" ? "小程序订单支付" : "推荐微信用户选择",
    },
  ];
...
export function normalizeOrderPayMethods(response, { platform = "app" } = {}) {
  ...
  const responseData =
    safeResponse.data && typeof safeResponse.data === "object"
      ? safeResponse.data
      : {};
  ...
  const normalized = rawOptions
    .map((item) => {
      ...
      return {
        value,
        label:
          String(safeItem.label || "").trim() || defaultPayMethodLabel(value),
        tip:
          String(safeItem.description || "").trim() ||
          "由后台支付中心统一控制",
      };
    })
    .filter(Boolean);
  ...
  return normalized.length > 0
    ? normalized
    : fallbackOrderPayMethods({ platform });
}
