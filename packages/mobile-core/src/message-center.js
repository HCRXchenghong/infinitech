...
function resolveSupportRuntimeTitle(
  runtime,
  fallback = DEFAULT_CONSUMER_MESSAGE_CENTER_UI.roleSupport,
) {
  const title = trimMessageText(runtime?.title);
...
export function createMessageCenterPage({
  ...
  return {
    ...
    computed: {
      ...
      showEmptyState() {
        return shouldShowConsumerMessageEmptyState(
          this.sessions,
          this.currentTab,
        );
      },
      showFooter() {
        return shouldShowConsumerMessageFooter(this.sessions, this.currentTab);
      },
    },
    onLoad() {
      uni.$off(
