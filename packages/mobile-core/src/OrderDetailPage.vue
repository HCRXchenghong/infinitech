    ...
    <scroll-view scroll-y class="scroll-content">
      ...
      <view v-if="order.bizType !== 'groupbuy' && order.deliveryInfo" class="info-card">
        ...
        <view v-if="shouldShowContactRider(order.status)" class="card-action">
          <button class="action-btn-inline primary" @tap="handleAction('contactRider', order)">
            联系骑手
          </button>
        </view>
      </view>
      ...
      <view class="info-card">
        ...
        <view class="product-list">
          <view v-for="(item, idx) in order.productList" :key="idx" class="product-item">
            <image
            ...
            <view class="product-info">
              ...
              <view v-if="item.spec || item.specification" class="product-spec">
        ...
        <view class="card-footer">
          ...
          <view
            v-if="order.discount !== undefined && order.discount > 0"
      ...
      <view class="info-card">
        ...
        <view v-if="order.status === 'pending_payment'" class="action-buttons">
          <button class="action-btn primary" @tap="handleAction('pay', order)">
            继续支付
          </button>
        </view>
        <view
          v-else-if="order.bizType === 'groupbuy' && order.status === 'paid_unused'"
          class="action-buttons"
        >
          <button class="action-btn primary" @tap="handleAction('voucher', order)">
            查看券码
          </button>
        </view>
        <view v-else-if="shouldShowRefund(order.status)" class="action-buttons">
          <button class="action-btn primary" @tap="handleAction('refund', order)">
            申请售后
          </button>
        </view>
