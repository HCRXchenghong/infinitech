...
function normalizeLocationContent(content, pickFirstDefined) {
  ...
  if (parsed) {
    ...
    const longitude = Number(
      pickFirstDefined(parsed.longitude, parsed.lng, parsed.lon),
    );
...
function normalizeAudioContent(content) {
  const parsed = safeParseJson(content);
  ...
  const durationSeconds = Math.max(
    1,
    Math.round(
      Number(parsed.durationSeconds || parsed.duration || parsed.length || 0) ||
        0,
    ),
  );
...
function resolvePreviewText(messageType, content) {
...
function normalizeMessageContent(messageType, content, pickFirstDefined) {
  ...
  if (messageType === "location") {
    return (
      normalizeLocationContent(content, pickFirstDefined) || {
        type: "text",
        text: String(content || "").trim(),
        preview: "[位置]",
        rawContent: content,
  ...
  const legacyLocation = normalizeLocationContent(content, pickFirstDefined);
  if (legacyLocation) {
...
export function createMessageChatPage({
  createSocket,
  config,
  fetchHistory,
  ...
  return {
    ...
    onLoad(query = {}) {
      ...
      this.loadSupportRuntimeConfig().finally(async () => {
    ...
    methods: {
      ...
      buildConversationPayload() {
        return {
          ...
          targetId:
            this.targetId || (this.isSupportConversation() ? "support" : ""),
          targetPhone: "",
      ...
      resolveMessageId(payload, fallback) {
        ...
        const explicitId = pickFirstDefined(
          safePayload.id,
          safePayload.uid,
          safePayload.tsid,
          safePayload.messageId,
          safePayload.mid,
        );
        if (
        ...
        const timestamp = this.resolveMessageTimestamp(
          safePayload.timestamp || safePayload.createdAt,
          Date.now(),
        );
        const senderRole =
          String(safePayload.senderRole || "unknown").trim() || "unknown";
        const senderId =
          String(safePayload.senderId || "unknown").trim() || "unknown";
        const messageType =
          String(
            safePayload.messageType || safePayload.type || "text",
          ).trim() || "text";
        const contentSeed = String(
          safePayload.content || safePayload.text || "",
        )
          .trim()
          .slice(0, 24)
          .replace(/\s+/g, "_")
        ...
        return (Array.isArray(list) ? list : [])
          .map((item, index) => {
            ...
            return {
              ...
              rawContent: Object.prototype.hasOwnProperty.call(
                item || {},
                "rawContent",
              )
                ? item.rawContent
      ...
      restoreLocalMessages() {
        try {
          ...
          if (
            ...
            cachedAt > 0 &&
            Date.now() - cachedAt > MESSAGE_CACHE_MAX_AGE
          ) {
      ...
      async loadServerHistory() {
        const hadServerHistory =
          this.messages.length > 0 && !this.historyFromLocalFallback;
        try {
      ...
      async initSocket() {
        ...
        const socketAccountKey = this.userId
      ...
      connectSocket(token) {
        ...
        sock.on("new_message", (payload) => {
          if (!payload || String(payload.chatId) !== String(this.roomId))
            return;
        ...
        sock.on("message_sent", (data) => {
          if (
            data &&
            data.chatId &&
            String(data.chatId) !== String(this.roomId)
          ) {
            return;
          }
      ...
      emitChatMessage(messageType, content) {
      ...
      send() {
        ...
        if (!text) return;
        if (!this.emitChatMessage("text", text)) return;
        this.draft = "";
      },
      ...
      initRecorderManager() {
        ...
        manager.onStop((res = {}) => {
          ...
          const durationMs =
            Number(res.duration || 0) || Date.now() - this.recordingStartedAt;
      ...
      uploadAndSendImage(tempFilePath) {
        ...
        uploadCommonAsset(tempFilePath, {
          ...
          .then((data) => {
            ...
            this.emitChatMessage("image", imageUrl);
          })
          .catch((err) => {
            uni.hideLoading();
      ...
      async sendLocation() {
        ...
        try {
          ...
          const address =
            String((geo && geo.address) || "").trim() ||
            `${lat.toFixed(6)},${lng.toFixed(6)}`;
          this.emitChatMessage("location", {
            address,
            latitude: lat,
            longitude: lng,
          });
      ...
      openLocation(message) {
        ...
        const address = String(
          (meta && meta.address) || (message && message.text) || "",
        ).trim();
