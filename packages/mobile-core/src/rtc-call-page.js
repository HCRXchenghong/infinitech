...
export function createRTCCallPage(options = {}) {
  ...
  const clearTimeoutFn =
    typeof options.clearTimeoutFn === "function" ? options.clearTimeoutFn : globalThis.clearTimeout;
  ...
  return {
    ...
    computed: {
      roleLabel() {
        if (this.targetRole === "rider") {
          return "联系骑手";
        }
        if (this.targetRole === "merchant" || this.targetRole === "shop") {
          return "联系商家";
        }
        return "站内语音";
      },
      ...
      mediaStatusText() {
        if (!this.mediaSupported) {
          return buildMediaStatusText("unsupported");
        }
    ...
    async onLoad(query) {
      ...
      try {
        ...
        this.errorMessage = error && error.message ? error.message : "初始化站内语音失败";
      }
    },
    onUnload() {
    ...
    methods: {
      ...
      clearTimeoutTimer() {
        ...
        clearTimeoutFn(this.timeoutTimer);
      ...
      scheduleTimeoutIfNeeded() {
        ...
        this.timeoutTimer = setTimeoutFn(() => {
          this.timeoutTimer = null;
          if (
            !this.session ||
      ...
      bindSessionHandlers() {
        return {
          ...
          rtc_status: async (payload) => {
            if (!payload || normalizeCallId(payload.callId) !== this.callId) {
              return;
            }
          ...
          rtc_signal: async (payload) => {
            try {
              ...
              this.errorMessage = error && error.message ? error.message : "RTC 媒体协商失败";
            }
          },
          rtc_error: (payload) => {
      ...
      async ensureMediaSession() {
        ...
        this.mediaSession = createRTCMediaSession({
          ...
          onIceCandidate: (candidate) => {
            if (!candidate || !this.session || !this.callId) {
              return;
            }
      ...
      async guardAction(handler) {
        ...
        try {
          ...
          this.errorMessage = error && error.message ? error.message : "通话状态更新失败";
        } finally {
      ...
      goChat() {
        if (!this.orderId || !this.targetRole) {
          if (uniApp && typeof uniApp.showToast === "function") {
            uniApp.showToast({ title: "缺少聊天上下文", icon: "none" });
          }
          return;
        }
