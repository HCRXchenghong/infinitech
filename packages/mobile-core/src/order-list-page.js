...
export function createOrderListPage(options = {}) {
  const {
    fetchAfterSalesList,
  ...
  const normalizeOrder =
    typeof mapOrderItem === "function" ? mapOrderItem : (item) => item;
  const normalizeAfterSales =
    typeof mapAfterSalesItem === "function" ? mapAfterSalesItem : (item) => item;
  ...
  return {
        ...
        if (this.currentTab === "delivering") {
          return this.orders.filter((order) => {
            if (order.bizType === "groupbuy") {
              return ["pending_payment", "paid_unused", "refunding"].includes(
                order.status,
              );
            }
    ...
    methods: {
      ...
      loadOrders() {
        ...
        if (!userId || typeof fetchOrders !== "function") {
          this.orders = [];
        ...
        fetchOrders(userId)
          .then((data) => {
            ...
            if (Array.isArray(data) && data.length > 0) {
              this.orders = data.map((order) => normalizeOrder(order));
            } else {
              this.orders = [];
            }
          })
      ...
      loadAfterSales(userId) {
        if (typeof fetchAfterSalesList !== "function") {
          this.afterSalesOrders = [];
          return;
        }
        fetchAfterSalesList(userId)
          .then((data) => {
            ...
            this.afterSalesOrders = list.map((item) => normalizeAfterSales(item));
          })
          .catch(() => {
            this.afterSalesOrders = [];
          });
      ...
      getStatusClass(order) {
        ...
        if (["pending", "pending_payment", "priced"].includes(status)) {
          return "status-pending";
        }
        if (["accepted", "delivering", "paid_unused", "refunding"].includes(status)) {
          return "status-delivering";
        }
        if (["completed", "redeemed", "refunded"].includes(status)) {
          return "status-completed";
        }
        return "status-cancelled";
      },
      getButtons(order) {
        ...
        if (status === "pending_payment") {
          return [{ text: "继续支付", primary: true, action: "pay" }];
        }
        ...
        if (["pending", "accepted", "delivering"].includes(status)) {
          return [{ text: "申请售后", primary: false, action: "refund" }];
        }
        if (status === "completed") {
      ...
      handleOnlineContact() {
        ...
        const context = buildOrderChatContext(this.currentOrder, this.contactType);
