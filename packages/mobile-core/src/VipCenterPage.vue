<template>
  <view class="vip-center-page">
    ...
    <template v-else>
      ...
      <view class="level-tabs"><view class="tabs-container"><view v-for="(level, index) in vipLevels" :key="`${level.name}-${index}`" class="tab-item" :class="{ active: activeTab === index }" @tap="switchTab(index)"><text>{{ level.name }}</text><view v-if="activeTab === index" class="tab-indicator"></view></view></view></view>
...
import { createProfileVipCenterPageOptions } from "./vip-center.js";
