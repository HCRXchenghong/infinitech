export function normalizeOrder(order) {
  ...
  return {
    ...
    id,
    orderNo:
      raw.orderNo || raw.order_no || raw.daily_order_id || String(id || ""),
...
export function normalizeIncomingMessage(payload, isSelf) {
  ...
  const timestamp = resolveMessageTimestamp(
    payload && (payload.timestamp || payload.createdAt),
    Date.now(),
  );
  const chatId =
    payload && (payload.chatId || payload.roomId || payload.conversationId)
      ? String(payload.chatId || payload.roomId || payload.conversationId)
      : "chat";
  const senderRole =
  ...
  return {
    id:
      payload && (payload.id || payload.uid || payload.tsid)
        ? payload.id || payload.uid || payload.tsid
        : fallbackId,
    ...
    order:
      type === "order"
        ? normalizeOrder(payload && (payload.order || payload.content))
        : null,
  };
}
...
export function formatOrderNo(order) {
  if (!order) return "--";
  return (
    order.orderNo || order.order_no || order.daily_order_id || order.id || "--"
  );
}
