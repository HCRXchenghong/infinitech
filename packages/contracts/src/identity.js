export const PrincipalTypes = Object.freeze({
  USER: "user",
  MERCHANT: "merchant",
  RIDER: "rider",
  ADMIN: "admin",
...
export function isUnifiedSessionClaimsShape(value) {
...
export function decodeBase64UrlToJSON(value) {
...
export function parseUnifiedTokenPayload(token) {
...
export function extractUnifiedPrincipalIdentity(payload, options = {}) {
  ...
  if (!normalizedPrincipalType) {
    ...
    } else if (normalizedRole === PrincipalTypes.USER || normalizedRole === PrincipalTypes.MERCHANT || normalizedRole === PrincipalTypes.RIDER) {
  ...
  return {
    ...
    legacyId: claimNumericString(payload, ["principal_legacy_id", "userId", "numericId", "legacyId"]),
    role: normalizedRole,
