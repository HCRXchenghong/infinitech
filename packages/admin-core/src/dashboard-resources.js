...
export function buildDashboardStatsCards(source = {}) {
  return createDefaultStatsCards().map((card) => ({
    ...card,
    value: formatNumber(source?.[card.key] || 0),
  }));
}

export function extractDashboardRankItems(payload = {}) {
...
export function getWeatherIconClass(icon) {
  ...
  return iconMap[normalizeText(icon)] || "default";
}
...
export function formatPresenceRole(role) {
  ...
  return roleMap[normalizeText(role).toLowerCase()] || "未知角色";
}
...
export function normalizeOnlinePresenceSample(sample) {
  ...
  return sample
    .map((entry, index) => {
      const socketId = normalizeText(entry?.socketId);
...
export function normalizeRedisHealth(raw) {
  ...
  return {
    ...
    mode:
      normalizeText(health.mode) ||
      (health.enabled ? "local-fallback" : "disabled"),
    host: normalizeText(health.host),
...
export function normalizeFallbackBuffer(raw) {
  ...
  return {
    ...
    startupDisabledPurged: Math.max(
      0,
      Math.floor(toFiniteNumber(buffer.startupDisabledPurged)),
    ),
    startupExpiredPruned: Math.max(
      0,
      Math.floor(toFiniteNumber(buffer.startupExpiredPruned)),
    ),
    startupOverflowPruned: Math.max(
      0,
      Math.floor(toFiniteNumber(buffer.startupOverflowPruned)),
    ),
...
export function extractHealthDetail(detail, key) {
  const source = String(detail || "");
  const match = source.match(new RegExp(`${key}=([^\\s|]+)`));
  return match ? normalizeText(match[1]) : "";
}

function resolveRuntimeServices(runtimeHealth) {
  const source = normalizeServiceHealthStatus(runtimeHealth);
  return Array.isArray(source.services) ? source.services : [];
}

export function formatDashboardRuntimeHealthStatus(overall) {
  if (overall === "ok") {
    return "已就绪";
  }
  if (overall === "degraded") {
...
export function buildDashboardRuntimeHealthSummary(runtimeHealth) {
  const services = resolveRuntimeServices(runtimeHealth);
  const serviceMap = new Map(services.map((item) => [item.key, item]));
  const segments = [
    serviceMap.has("go")
      ? `Go ${serviceMap.get("go")?.status === "up" ? "已就绪" : "异常"}`
      : "",
    serviceMap.has("socket")
      ? `Socket ${serviceMap.get("socket")?.status === "up" ? "已就绪" : "异常"}`
      : "",
    serviceMap.has("redis")
      ? `Redis ${serviceMap.get("redis")?.status === "up" ? "已就绪" : "异常"}`
      : "",
  ].filter(Boolean);
  return segments.join(" · ") || "系统探针暂未返回详情";
}

export function buildDashboardPushWorkerSummary(runtimeHealth) {
  const services = resolveRuntimeServices(runtimeHealth);
  const goService = services.find((item) => item.key === "go") || null;
  const detail = normalizeText(goService?.detail);
