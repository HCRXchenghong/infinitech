  ...
  <view class="bill-page">
    <view class="top-shell" :style="{ paddingTop: topPadding + 'px' }">
      <view class="top-bar">
        ...
        <view class="title-group">
          ...
          <text class="top-subtitle">骑手 IF-Pay 账户流水</text>
        </view>
    ...
    <view class="filter-shell">
      <view class="time-card">
        ...
        <picker v-if="rangeMode === 'month'" mode="date" fields="month" :value="monthValue" @change="onMonthChange">
      ...
      <view v-else class="tx-list">
        ...
        >
          ...
          <view class="tx-right">
            ...
            <text class="tx-status">{{ statusLabel(tx.status, tx.type) }}</text>
          </view>
        </view>
      </view>
    </view>

    <view class="modal-mask" v-if="detailVisible" @tap.self="detailVisible = false">
      <view class="detail-sheet">
        <view class="sheet-handle"></view>
        <text class="detail-title">账单详情</text>
        <view class="detail-row" v-for="row in detailRows" :key="row.label">
