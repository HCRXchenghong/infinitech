...
export default {
  ...
  methods: {
    ...
    extractErrorMessage(err) {
      const rawMessage = (err && err.data && err.data.error) || (err && err.error) || (err && err.message) || ''
    ...
    async loadAvailableCoupons(shopId) {
      try {
        ...
        const htmlPayload = (err && err.data && err.data.data) || (err && err.data)
    ...
    submitOrder() {
      ...
      const bizTypeRaw = String((this.shop && this.shop.merchantType) || (this.shop && this.shop.merchant_type) || (this.shop && this.shop.orderType) || '').toLowerCase()
