...
export default {
  ...
  methods: {
    ...
    formatOrderData(data) {
      ...
      return {
        id: data.id,
        shopName: data.shopName || data.shop_name || (data.shop && data.shop.name) || '',
        shopLogo: data.shopLogo || (data.shop && data.shop.logo) || '',
    ...
    async handleSubmit() {
      ...
      try {
        ...
        const message = (error && error.data && error.data.error) || (error && error.error) || '提交失败，请稍后重试'
