...
function orderStatusClass(status, bizType, isAfterSales) {
  if (isAfterSales) {
    const lowerStatus = String(status || '').toLowerCase();
    if (lowerStatus === 'pending') return 'status-pending';
    if (lowerStatus === 'processing') return 'status-delivering';
    if (lowerStatus === 'approved' || lowerStatus === 'completed') return 'status-completed';
    if (lowerStatus === 'rejected') return 'status-cancelled';
    return 'status-cancelled';
  }

  if (bizType === 'groupbuy') {
    if (['pending_payment'].includes(status)) return 'status-pending';
    if (['paid_unused', 'refunding'].includes(status)) return 'status-delivering';
    if (['redeemed', 'refunded'].includes(status)) return 'status-completed';
    return 'status-cancelled';
  }
...
export function mapOrderItem(order) {
  ...
  return {
    ...
    shopId: order.shopId || order.shop_id || (order.shop && order.shop.id),
    shopName: order.shopName || order.shop_name || (order.shop && order.shop.name) || '未知商家',
    shopLogo: order.shopLogo || (order.shop && order.shop.logo) || '/static/images/default-shop.svg',
    bizType,
    status,
    statusClass: orderStatusClass(status, bizType, false),
...
export function mapAfterSalesItem(item) {
  ...
  return {
    ...
    status: status,
    statusText: item.statusText || status || '待处理',
    statusClass: orderStatusClass(status, 'takeout', true),
