    ...
    async loadFavorites(reset = false) {
      ...
      try {
        ...
        const normalizedList = list.map((item, index) => {
          const keyCandidate = item.favoriteId || item.id
          return {
            ...item,
            favKey: String(keyCandidate || `${this.page}_${index}`)
          }
        })

        if (reset) {
          this.items = normalizedList
        } else {
          this.items = this.items.concat(normalizedList)
        }
