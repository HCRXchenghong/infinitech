<template>
  <view class="page category-page">
    <PageHeader :title="categoryName" :showSearch="true" @search="goSearch" />
...
export default {
  ...
  computed: {
    filteredShops() {
      ...
      if (this.categoryName && this.categoryName !== '全部分类') {
        shops = shops.filter((shop) => this.matchCategory(shop, this.categoryName))
      }
  ...
  onLoad(query) {
    this.categoryName = decodeURIComponent(query.category || '全部分类')
    this.loadShops()
  },
  methods: {
    async loadShops() {
      ...
      try {
        ...
        this.allShops = Array.isArray(data) ? data : []
      } catch (err) {
    ...
    matchCategory(shop, categoryName) {
      const tags = Array.isArray(shop.tags) ? shop.tags : []
      const category = shop.businessCategory || shop.category || ''

      if (category && (category.includes(categoryName) || categoryName.includes(category))) {
        return true
      }

      return tags.some(tag => tag.includes(categoryName) || categoryName.includes(tag))
    },
    goSearch() {
