  ...
  return new Promise((resolve, reject) => {
    uni.request({
      ...
      fail(err) {
        ...
        const isNetworkError = errMsg.includes('fail') || errMsg.includes('connect')
    ...
    uni.uploadFile({
      ...
      success(res) {
        ...
        reject({
          statusCode: res.statusCode,
          data: parsed,
          error: (parsed && parsed.error) || '上传失败'
        })
      },
      fail(err) {
        reject({
          error: (err && err.errMsg) || '上传失败'
        })
      }
    })
  })
...
export const fetchFeaturedProducts = async () => {
  try {
    ...
    const rawData = (error && error.data && error.data.data) || (error && error.data)
...
function buildWeatherTips(weatherData = {}) {
  ...
  const condition = String((weatherData && weatherData.weather) || (weatherData && weatherData.weather_main) || '')
