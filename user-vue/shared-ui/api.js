    ...
    uni.request({
      ...
      fail(err) {
        ...
        const isNetworkError =
          errMsg.includes("fail") || errMsg.includes("connect");
...
export const fetchShopDetail = async (shopId) => {
  // 详情也优先取新数据，失败再回退本地
  const data = await syncService.getData(
    "shops",
    { id: shopId },
    { preferFresh: true },
  );
...
export const listDiningBuddyParties = (params = {}) =>
  request({
...
export const fetchAfterSalesList = (userId) =>
  request({
    url: `/api/after-sales/user/${encodeURIComponent(userId)}`,
  });

export const fetchGroupbuyVouchers = (params = {}) =>
  request({
...
export const fetchVoucherQRCode = (voucherId) =>
  request({
    url: `/api/groupbuy/vouchers/${encodeURIComponent(voucherId)}/qrcode`,
    method: "GET",
  });

export const fetchUserCoupons = (params = {}) =>
  request({
...
export const readAuthorizationHeader = () => {
  const token = readStoredBearerToken(uni, ["token", "access_token"]);
...
export const addUserFavorite = (userId, shopId) =>
  request({
    url: `/api/user/${userId}/favorites`,
    method: "POST",
    data: { shopId },
...
export const deleteUserFavorite = (userId, shopId) =>
  request({
    url: `/api/user/${userId}/favorites/${shopId}`,
    method: "DELETE",
  });

export const fetchUserFavoriteStatus = (userId, shopId) =>
  request({
...
export const consumeWechatSession = (token) =>
  request({
...
export const wechatBindLogin = (payload) =>
  request({
...
export const requestSMSCode = (phone, scene, extra = {}) =>
  request({
...
export const verifySMSCodeCheck = (phone, scene, code) =>
  request({
...
export const changeUserPhone = (userId, payload) =>
  request({
...
export const recordPhoneContactClick = (payload) =>
  request({
...
export const fetchPublicCharitySettings = () =>
  request({
    url: "/api/public/charity-settings",
    method: "GET",
  });

export const fetchPublicVIPSettings = () =>
  request({
...
export const fetchMenuItems = async (shopId) => {
...
function buildWeatherTips(weatherData = {}) {
  const indices = (weatherData && weatherData.life_indices) || {};
  const umbrellaAdvice = indices && indices.umbrella && indices.umbrella.advice;
  const uvAdvice = indices && indices.uv && indices.uv.advice;
  const travelAdvice = indices && indices.travel && indices.travel.advice;
  ...
  const condition = String(
    (weatherData && weatherData.weather) ||
      (weatherData && weatherData.weather_main) ||
      "",
  );
...
function getAirQualityText(weatherData = {}) {
  if (weatherData && weatherData.aqi_category) return weatherData.aqi_category;
  if (weatherData && weatherData.air_quality) return weatherData.air_quality;

  const aqi = Number(weatherData && weatherData.aqi);
