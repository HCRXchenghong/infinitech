...
function normalizeServiceType(value) {
  ...
  if (!text) return ''
  if (text === 'errand_buy' || text === 'buy' || text === 'purchase') return 'errand_buy'
  if (text === 'errand_deliver' || text === 'deliver' || text === 'delivery') return 'errand_deliver'
  if (text === 'errand_pickup' || text === 'pickup' || text === 'pick_up') return 'errand_pickup'
  if (text === 'errand_do' || text === 'do' || text === 'task') return 'errand_do'
  return text
}
...
export function getErrandServiceMeta(serviceType) {
  return ERRAND_SERVICE_META[normalizeServiceType(serviceType)] || ERRAND_SERVICE_META.errand_generic
}
...
export function mapErrandOrderDetail(order = {}) {
  ...
  const location = parseJsonObject(order.errandLocation || order.errand_location)
  const requirements = parseJsonObject(order.errandRequirements || order.errand_requirements)
  const serviceType = normalizeServiceType(order.serviceType || order.service_type) || 'errand_generic'
  const meta = getErrandServiceMeta(serviceType)
  const statusText = pickText(order.statusText, order.status_text, order.status, '待接单')
  const pickup = pickText(location.pickup, request.buyAddress, request.pickupAddress, request.pickup)
  ...
  return {
    ...
    createdAtText: formatDateTime(order.createdAt || order.created_at || order.time)
  }
}
