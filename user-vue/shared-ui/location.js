  ...
  return new Promise((resolve, reject) => {
    ...
    ensureLocationPermission()
      .then(() => {
        // #ifdef APP-PLUS
        try {
          if (typeof plus !== 'undefined' && plus && plus.geolocation) {
            plus.geolocation.getCurrentPosition(
              (pos) => {
                const coords = pos.coords || {}
                const latitude = coords.latitude !== undefined && coords.latitude !== null ? coords.latitude : pos.latitude
                const longitude = coords.longitude !== undefined && coords.longitude !== null ? coords.longitude : pos.longitude
                const parsedLatitude = Number(latitude)
                const parsedLongitude = Number(longitude)
                if (!Number.isFinite(parsedLatitude) || !Number.isFinite(parsedLongitude)) {
...
function ensureAndroidRuntimePermission() {
  return new Promise((resolve, reject) => {
    // #ifdef APP-PLUS
    try {
      if (typeof plus === 'undefined' || !plus || !plus.os || plus.os.name !== 'Android') {
        resolve()
        return
      }
      ...
      plus.android.requestPermissions(
        ...
        (result) => {
          const deniedAlways = Array.isArray(result && result.deniedAlways) ? result.deniedAlways : []
          const deniedPresent = Array.isArray(result && result.deniedPresent) ? result.deniedPresent : []
