export function createWalletBillsPageLogic({
  request,
  ...
  const resolvedFilterOptions = Array.isArray(filterOptions) && filterOptions.length > 0
    ? filterOptions.map((item) => Object.assign({}, item))
    : defaultFilterOptions.map((item) => Object.assign({}, item))
  const defaultTxTypeLabelMap = {
    payment: '订单支付',
    refund: '订单退款',
    recharge: '余额充值',
    withdraw: '余额提现',
  ...
  const customTxTypeLabelMap =
    txTypeLabels && typeof txTypeLabels === 'object'
      ? txTypeLabels
      : null
  const defaultTxTypeIconMap = {
    payment: '支',
    refund: '退',
    recharge: '充',
    withdraw: '提',
  ...
  const customTxTypeIconMap =
    txTypeIcons && typeof txTypeIcons === 'object'
      ? txTypeIcons
      : null
  ...
  return {
    data() {
      ...
      return {
        ...
        incomeAmount: 0,
        expenseAmount: 0,
        filterOptions: resolvedFilterOptions.map((item) => Object.assign({}, item))
      }
    },
    computed: {
      topPadding() {
    ...
    methods: {
      ...
      txTypeLabel(type) {
        ...
        return (customTxTypeLabelMap && customTxTypeLabelMap[normalized]) || defaultTxTypeLabelMap[normalized] || (type || '资产变动')
      },
      txTypeIcon(type) {
        const normalized = String(type || '').toLowerCase()
        return (customTxTypeIconMap && customTxTypeIconMap[normalized]) || defaultTxTypeIconMap[normalized] || '资'
      },
      statusLabel(status, type) {
        if (typeof formatStatusLabel === 'function') {
          const customStatusText = formatStatusLabel(status, type)
          if (typeof customStatusText === 'string' && customStatusText.trim()) {
