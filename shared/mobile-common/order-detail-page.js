  ...
  return {
    ...
    methods: {
      ...
      formatOrderData(data) {
        const shopId = data.shopId || data.shop_id || (data.shop && data.shop.id)
        const shopName = data.shopName || data.shop_name || (data.shop && data.shop.name)
        const shopLogo = data.shopLogo || (data.shop && data.shop.logo)
        const shopPhone = data.shopPhone || (data.shop && data.shop.phone) || data.customer_phone
      ...
      parseStatus(status, bizType = 'takeout') {
        if (typeof status === 'string') {
          ...
          if (bizType === 'groupbuy') {
            if (['pending_payment', 'paid_unused', 'redeemed', 'refunding', 'refunded', 'expired', 'cancelled'].includes(normalized)) {
              return normalized
            }
            if (normalized.includes('核销')) return normalized.includes('已') ? 'redeemed' : 'paid_unused'
            if (normalized.includes('退款')) return normalized.includes('中') ? 'refunding' : 'refunded'
            if (normalized.includes('过期')) return 'expired'
            return 'paid_unused'
          }
          if (normalized === 'pending_payment') return 'pending_payment'
          if (normalized === 'completed' || normalized.includes('送达') || normalized.includes('完成')) return 'completed'
          if (normalized === 'cancelled' || normalized.includes('取消')) return 'cancelled'
          if (normalized === 'accepted' || normalized === 'priced' || normalized.includes('配送') || normalized.includes('进行') || normalized.includes('接单')) {
            return 'delivering'
          }
          if (normalized === 'pending' || normalized.includes('待')) return 'pending'
        }
      ...
      async continuePayOrder(order) {
        ...
        try {
          ...
          if (shouldLaunchClientPayment(result)) {
            ...
            try {
              await invokeClientPayment(result, platform)
            } finally {
              uni.hideLoading()
            }
          }
      ...
      handleOnlineContact() {
        ...
        if (this.contactType === 'rider') {
          ...
          name = ((order.deliveryInfo && order.deliveryInfo.rider) ? order.deliveryInfo.rider.split(' ')[0] : '') || '骑手'
          role = 'rider'
      ...
      handleConfirmPhone() {
        ...
        if (this.contactType === 'rider') {
          ...
          phoneNumber = phoneMatch ? phoneMatch[0] : (((order.deliveryInfo && order.deliveryInfo.contact) || '10086'))
        } else {
