...
async function main() {
  ...
  const manifest = {
    ...
    files: {
      ...
      secretRotationGateReport: path.join(reportsDir, 'secret-rotation-gate.json'),
      realtimeAcceptanceReport: path.join(reportsDir, 'realtime-acceptance.json'),
      pushRealDevicePrepReport: path.join(reportsDir, 'push-real-device-prep.json'),
      failureDrillPrepReport: path.join(reportsDir, 'failure-drill-prep.json'),
      rtcRealDevicePrepReport: path.join(reportsDir, 'rtc-real-device-prep.json'),
    ...
    commands: {
      ...
      secretRotationEvidenceGate: 'node scripts/security/check-secret-rotation-evidence.mjs',
      realtimeAcceptance: 'node scripts/realtime-acceptance-gate.mjs',
      pushRealDevicePrep: 'node scripts/push-real-device-prep.mjs',
      failureDrillPrep: 'node scripts/failure-drill-prep.mjs',
      evidenceGate: 'node scripts/release-evidence-gate.mjs',
      rtcRealDevicePrep: 'node scripts/rtc-real-device-prep.mjs',
    ...
    executionOrder: [
      ...
      '5. Run push-real-device-prep and store the report in reports/push-real-device-prep.json.',
      '6. Run failure-drill-prep and store the report in reports/failure-drill-prep.json.',
      '7. Run security/check-secret-rotation-evidence and store the report in reports/secret-rotation-gate.json.',
      '8. Run realtime-acceptance-gate and store the report in reports/realtime-acceptance.json.',
      '9. Run release-evidence-gate and store the report in reports/evidence-gate.json.',
      '10. Run rtc-real-device-prep and store the report in reports/rtc-real-device-prep.json.',
      '11. Run release-manual-attestation-prefill to generate manual-attestation/prefilled.json from the drill reports.',
      '12. Complete manual-attestation/completed.json with real-device push, RTC, and failure-recovery evidence.',
      '13. Run release-final-signoff and store the report in reports/final-signoff.json.',
    ],
    templateExitCode,
  };
