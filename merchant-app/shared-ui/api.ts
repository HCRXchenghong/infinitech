...
function parseBaseUrl(url: string): { protocol: string; host: string; port: string } | null {
...
function buildCandidateBaseUrls(currentBaseUrl: string): string[] {
  ...
  if (parsed) {
    result.push(`${parsed.protocol}${parsed.host}${parsed.port ? `:${parsed.port}` : ''}`)
...
async function resolveReachableBaseUrl(currentBaseUrl: string): Promise<string | null> {
...
function normalizeResourceUrl(url: string, baseUrl: string): string {
...
function uploadFileByBaseUrl(filePath: string, baseUrl: string): Promise<UploadResult> {
    ...
    uni.uploadFile({
      ...
      success(res: any) {
        ...
        try {
          ...
          reject(buildError('上传响应解析失败', { data: res.data, statusCode: res.statusCode }))
          return
        }

        const url = normalizeResourceUrl(String(parsed?.url || ''), baseUrl)
        ...
        reject(
          buildError(parsed?.error || `上传失败: ${res.statusCode}`, {
            data: parsed,
      ...
      fail(err: any) {
        reject(buildError(err?.errMsg || '上传失败', { data: err }))
      },
    })
  })
}
...
export async function uploadImage(filePath: string): Promise<UploadResult> {
  ...
  try {
    ...
    const errMsg = String(err?.error || err?.message || err?.data?.errMsg || '')
...
function apiPost<T = any>(url: string, data?: Record<string, any>, auth = true) {
...
function apiDelete<T = any>(url: string, data?: Record<string, any>, auth = true) {
  return request<T>({ url, method: 'DELETE', data, auth })
}

// Auth
export const merchantLogin = (credentials: { phone: string; code?: string; password?: string }) =>
  apiPost('/api/auth/merchant/login', credentials, false)

export const requestSMSCode = (phone: string, scene: string = 'merchant_login') =>
  apiPost('/api/request-sms-code', { phone, scene }, false)

export const verifySMSCodeCheck = (payload: { phone: string; code: string; scene?: string }) =>
  apiPost('/api/verify-sms-code-check', payload, false)

export const merchantSetNewPassword = (payload: { phone: string; code: string; password: string }) =>
...
export const fetchShopDetail = (shopId: string | number) => apiGet(`/api/shops/${shopId}`)

export const fetchShopMenu = (shopId: string | number) => apiGet(`/api/shops/${shopId}/menu`)

export const updateShop = (shopId: string | number, payload: Record<string, any>) =>
...
// Orders
export const fetchOrders = (params: { page?: number; limit?: number; status?: string; search?: string } = {}) =>
  apiGet<{ orders: any[]; total: number }>('/api/orders', params)

export const fetchOrderDetail = (orderId: string | number) => apiGet(`/api/orders/${orderId}`)

export const dispatchOrder = (orderId: string | number) => apiPost(`/api/orders/${orderId}/dispatch`, {})

export const pickupOrder = (orderId: string | number) => apiPost(`/api/orders/${orderId}/pickup`, {})

export const deliverOrder = (orderId: string | number) => apiPost(`/api/orders/${orderId}/deliver`, {})
...
// Products
export const fetchProducts = (params: { shopId: string | number; categoryId?: string | number }) =>
  apiGet<any[]>('/api/products', params)

export const fetchProductDetail = (productId: string | number) =>
  apiGet(`/api/products/${productId}`)

export const createProduct = (payload: Record<string, any>) => apiPost('/api/products', payload)

export const updateProduct = (productId: string | number, payload: Record<string, any>) =>
...
// Banners / Decoration
export const fetchBanners = (shopId: string | number) => apiGet<any[]>('/api/banners', { shopId })

export const createBanner = (payload: Record<string, any>) => apiPost('/api/banners', payload)

export const updateBanner = (bannerId: string | number, payload: Record<string, any>) =>
  apiPut(`/api/banners/${bannerId}`, payload)

export const deleteBanner = (bannerId: string | number, shopId: string | number) =>
  apiDelete(`/api/banners/${bannerId}`, { shopId })

// Coupons
export const fetchShopCoupons = (shopId: string | number) => apiGet(`/api/shops/${shopId}/coupons`)

export const fetchActiveCoupons = (shopId: string | number) => apiGet(`/api/shops/${shopId}/coupons/active`)

export const fetchCouponDetail = (couponId: string | number) => apiGet(`/api/coupons/${couponId}`)

export const createCoupon = (payload: Record<string, any>) => apiPost('/api/coupons', payload)

export const updateCoupon = (couponId: string | number, payload: Record<string, any>) =>
  apiPut(`/api/coupons/${couponId}`, payload)

export const deleteCoupon = (couponId: string | number) => apiDelete(`/api/coupons/${couponId}`, {})
...
export const updateReview = (reviewId: string | number, payload: Record<string, any>) =>
  apiPut(`/api/reviews/${reviewId}`, payload)

export const deleteReview = (reviewId: string | number) => apiDelete(`/api/reviews/${reviewId}`)

// After-sales
export const fetchAfterSales = (params: { page?: number; limit?: number; status?: string; search?: string } = {}) =>
  apiGet('/api/after-sales', params)

export const updateAfterSalesStatus = (id: string | number, payload: Record<string, any>) =>
  apiPut(`/api/after-sales/${id}/status`, payload)

// Wallet
export const fetchWalletBalance = (userId: string | number, userType: 'customer' | 'rider' | 'merchant' = 'merchant') =>
