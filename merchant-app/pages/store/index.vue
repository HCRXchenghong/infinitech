...
function merchantWalletId() {
  const profile: any = uni.getStorageSync('merchantProfile') || {}
  const id = String(profile.id || profile.role_id || profile.userId || '').trim()
...
async function loadWallet() {
  ...
  try {
    const res: any = await fetchWalletBalance(userId, 'merchant')
    balance.value = Number(res?.balance || 0)
    frozenBalance.value = Number(res?.frozenBalance || 0)
  } catch (_err) {
    balance.value = 0
    frozenBalance.value = 0
  } finally {
...
async function loadData(force = false) {
  ...
  const active = shops.value.find((item: any) => String(item.id) === String(currentId)) || ctx.currentShop || null
  currentShop.value = active
...
async function toggleBusiness(e: any) {
  ...
  try {
    ...
    currentShop.value = { ...currentShop.value, isActive: next }
